    ...
    public class TradeDateService
    {
        ...
        public async Task<IEnumerable<TradeDate>> GetTradeDates(int count, int lastdate)
        {
            ...
            var results = await connection.QueryAsync<TradeDate>(
                "SELECT * FROM akshare.tool_trade_date_hist_sina ttdhs where t_date<=@today order by t_date desc limit @count ",
        ...
        public int GetLatestTradeDate()
        {
            ...
            var result = connection.ExecuteScalar<int>(
                "SELECT t_date FROM akshare.tool_trade_date_hist_sina ttdhs where t_date<=@date order by t_date desc limit 1",
                new { date = date });
            return result;
        }
        // 判断某日期是否为交易日
        public async Task<bool> IsTradeDate(int date)
        {
            ...
            var result = await connection.QueryFirstOrDefaultAsync<int?>(
                "SELECT 1 FROM akshare.tool_trade_date_hist_sina ttdhs where t_date=@date limit 1",
                new { date = date });
